...
trait Logging { fn logging(&self) -> String ; }
...
#[derive(Debug)]
struct LoggingEnabled<S: State> { prev: S, level: String }
impl<S> Tracing for LoggingEnabled<S>
where
    S: State + Tracing
{
    fn tracing(&self) -> bool { self.prev.tracing() } 
}
...
impl<S: State> EnableTracing for S {
    fn enable_tracing(self, enabled: bool) -> TracingEnabled<S> {
        TracingEnabled { prev: self, enabled }
    }
}
trait Tracing { fn tracing(&self) -> bool; }
impl<S: State> Tracing for TracingEnabled<S> {
    fn tracing(&self) -> bool { self.enabled }
}
impl<S: State> State for TracingEnabled<S> {}
#[derive(Debug)]
struct TracingEnabled<S: State> { prev: S, enabled: bool }
impl<S> Logging for TracingEnabled<S>
where
    S: State + Logging
{
    fn logging(&self) -> String { self.prev.logging() } 
}
